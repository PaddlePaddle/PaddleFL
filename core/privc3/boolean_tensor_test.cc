...
public:
  ...
  gen(const std::vector<size_t> &shape) {
...
TEST_F(BooleanTensorTest, reveal1_test) {
  ...
  _t[0] = std::thread([&]() {
    ContextHolder::template run_with_context(
        _exec_ctx.get(), _mpc_ctx[0], [&]() { b0.reveal_to_one(0, p.get()); });
  });

  _t[1] = std::thread([&]() {
    ContextHolder::template run_with_context(
        _exec_ctx.get(), _mpc_ctx[1], [&]() { b1.reveal_to_one(0, nullptr); });
  });

  _t[2] = std::thread([&]() {
    ContextHolder::template run_with_context(
        _exec_ctx.get(), _mpc_ctx[2], [&]() { b2.reveal_to_one(0, nullptr); });
  });
  for (auto &t : _t) {
    t.join();
  }
...
TEST_F(BooleanTensorTest, reveal2_test) {
  ...
  _t[1] = std::thread([&]() {
    ContextHolder::template run_with_context(_exec_ctx.get(), _mpc_ctx[1],
                                             [&]() { b1.reveal(p[1].get()); });
  });

  _t[2] = std::thread([&]() {
    ContextHolder::template run_with_context(_exec_ctx.get(), _mpc_ctx[2],
                                             [&]() { b2.reveal(p[2].get()); });
  });
  for (auto &t : _t) {
    t.join();
...
TEST_F(BooleanTensorTest, bit_extract_test3) {
  ...
  std::shared_ptr<TensorAdapter<int64_t>> sl[3] = {gen(shape), gen(shape),
                                                   gen(shape)};
