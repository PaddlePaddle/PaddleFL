...
public:
    void ComputeImpl(const framework::ExecutionContext &ctx) const override{
        ...
        mpc::MpcInstance::mpc_instance()->mpc_protocol()->mpc_operators()->elementwise_mul(in_x_t, in_y_t, out_t, axis);
    }
};
...
public:
    void ComputeImpl(const framework::ExecutionContext &ctx) const override {
        ...
        if (dx && dy && (in_x_t->dims().size() == in_y_t->dims().size())) {
            ...
            // dx = dout * y
            // dy = dout * x
            mpc::MpcInstance::mpc_instance()->mpc_protocol()->mpc_operators()->elementwise_mul(dout, in_y_t, dx);
            mpc::MpcInstance::mpc_instance()->mpc_protocol()->mpc_operators()->elementwise_mul(dout, in_x_t, dy);
        }

        mpc::MpcInstance::mpc_instance()->mpc_protocol()->mpc_operators()->elementwise_mul_grad(in_x_t, in_y_t, dout, dx, dy, axis);
