...
class PrivcProcess(Process):
    """
    Extends from Process, evaluate the computation party in privc.
    """
...
class OpTest(unittest.TestCase):
    ...
    def reconstruct(self, shares):
        return (shares[0] + shares[1]) * 2**-OpTest.scaling_factor

    def lazy_reconstruct(self, shares):
        return shares * 2 * 2**-OpTest.scaling_factor

    def lazy_share(self, plain):
        return np.array(plain * 2**OpTest.scaling_factor / 2.0).astype("int64")

    def multi_party_run(self, **kwargs):
        """
        Run 2 parties with target function or other additional arguments.
        :param kwargs:
        :return:
        """
        r = redis.Redis(host=self.server, port=int(self.port))
        ...
        for role in range(self.party_num):
            ...
            partys.append(PrivcProcess(target=target, kwargs=kwargs))
            partys[-1].start()
        for party in partys:
            party.join()
            if party.exception:
