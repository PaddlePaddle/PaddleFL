...
class TestOpMul(test_op_base.TestOpBase):
    ...
    def mat_mul(self, **kwargs):
        ...
        out = pfl_mpc.layers.mul(data_1, data_2)
    ...
    def mat_mul2(self, **kwargs):
        ...
        out = pfl_mpc.layers.mul(data_1, data_2, x_num_col_dims = 2, y_num_col_dims = 2)
        exe = fluid.Executor(place=fluid.CPUPlace())
        results = exe.run(feed={'data_1': d_1, 'data_2': d_2}, fetch_list=[out])
    ...
    def test_mul2(self):
        data_1 = np.arange(0, 120).reshape((2, 3, 4, 5))
        data_1_shares = mpc_du.make_shares(data_1)

        data_2 = np.arange(0, 120).reshape((5, 4, 3, 2))
        data_2_shares = mpc_du.make_shares(data_2)

        return_results = Manager().list()
        ret = self.multi_party_run(target=self.mat_mul2,
                                   data_1=data_1_shares,
                                   data_2=data_2_shares,
                                   return_results=return_results)
        self.assertEqual(ret[0], True)
