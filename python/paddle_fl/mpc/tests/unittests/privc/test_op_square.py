...
class TestOpAdd(test_op_base.TestOpBase):
    ...
    def elementwise_square(self, **kwargs):
        """
        Normal case.
        :param kwargs:
        :return:
        ...
        out = pfl_mpc.layers.square(data_1)
    ...
    def test_square(self):
        ...
        ret = self.multi_party_run(target=self.elementwise_square,
                                   data_1=data_1_all2shares,
                                   return_results=return_results)
        self.assertEqual(ret[0], True)
        revealed = self.reconstruct(np.array(return_results))
        expected_out = np.array([[0, 1], [4, 9], [16, 25]])
