...
class Solution(test_op_base.TestOpBase):
    def findMedianSortedArrays(self, **kwargs):
        ...
        role = 0
        d_1 = np.load('data_C0_P1.npy',allow_pickle=True)
        d_2 = np.load('data_C0_P2.npy',allow_pickle=True)
        d_3 = np.load('data_C0_P3.npy',allow_pickle=True)
        ...
        if totalLength % 2 == 1:
            ...
            d_tmp = np.load('data_C0_tmp.npy',allow_pickle=True)
            results = exe.run(feed={'x': tmp[0],'y':d_tmp,'zero': d_zero}, fetch_list=[math_mul])
            np.save('result_C0.npy', results[0])

    def test_mid3_C0(self):
        ret = self.multi_party_run0(target=self.findMedianSortedArrays)
