...
class Solution(test_op_base.TestOpBase):
    def findMedianSortedArrays(self, **kwargs):
        ...
        role = 2
        d_1 = np.load('data_C2_P1.npy',allow_pickle=True)
        d_2 = np.load('data_C2_P2.npy',allow_pickle=True)
        d_3 = np.load('data_C2_P3.npy',allow_pickle=True)
        ...
        if totalLength % 2 == 1:
            ...
            d_tmp = np.load('data_C2_tmp.npy',allow_pickle=True)
            results = exe.run(feed={'x': tmp[0],'y':d_tmp,'zero': d_zero}, fetch_list=[math_mul])
            np.save('result_C2.npy', results[0])

    def test_mid3_C2(self):
        ret = self.multi_party_run2(target=self.findMedianSortedArrays)
