...
class TestOpMax3_C1(test_op_base.TestOpBase):

    def max3_C1(self, **kwargs):
        ...
        if results0[0] == 1:
            ...
            results1 = exe.run(feed={'x': d_tmp1[0], 'y': d_2, 'zero': d_zero}, fetch_list=[op_gt])
        if results1[0] == 1:
            np.save('result_C1.npy', d_3)
        else:
            if results0[0] == 1:
                np.save('result_C1.npy', d_1)
            else:
                np.save('result_C1.npy', d_2)

    def test_max3_C1(self):
        ret = self.multi_party_run1(target=self.max3_C1)
