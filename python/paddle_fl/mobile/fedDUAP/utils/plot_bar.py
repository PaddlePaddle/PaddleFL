...
fig_dir = f"./fig/{model_name}/cmp_prunerate_l5_interval30"
isPDF = False
...
def plot_single_line(file_name, name, percent=0):
    ...
    messages.append("name: {:15}, percent: {:2d} acc: {:.4f} ({:.4f})  finish time {:.1f} final_flop: {} ({:.1f}%) final parameters {} "
                    "({:.1f}%) ".format(name, percent,  acc, acc-base_acc, time[-1], final_flop, final_flop/base_flop * 100,
                                                      final_weight, final_weight/base_size * 100))
    print(messages[-1])
