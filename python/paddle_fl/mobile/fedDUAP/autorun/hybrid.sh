...
echo ${model}
  for share_percent in ${share_percents[*]}; do
    if [[ ${share_percent} == 10 || ${share_percent} == 0 ]]; then
      nohup python ../hybridmain.py --iid 0 --share_percent ${share_percent} --unequal ${unequal} --dataset ${dataset} \
...
#nohup python ../hybridmain.py --iid 1 --share_percent 1 --unequal ${unequal} --result_dir ${dir_name} --epochs ${epochs} \
